...
/*   Updated: 2024/04/21 03:53:31 by sbansacc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "./../ft_printf_bonus.h"

int	ft_putuphexa(unsigned int hex, int len_max)
{
	char	*base;
	...
	{
		len_max--;
		if (ft_putuphexa(hex / 16, len_max) == (-1))
			return (-1);
	}
...
int	print_uphash(t_flags flags, unsigned int hex, int **printed_char)
{
...
int	print_zero_uphash(t_flags flags, int len, unsigned int hex,
	int *printed_char)
{
	...
	if (print_uphash(flags, hex, &printed_char) == (-1))
		return (-1);
	if (flags.has_hash && hex != 0)
		len -= 2;
	if (!flags.has_minus && flags.has_zero && flags.width && !flags.has_dot)
...
int	print_uphexa_bonus(int argument, t_flags flags)
{
	...
	if (print_width(flags, len, &printed_char) == (-1))
		return (-1);
	if (print_zero_uphash(flags, len, hex, &printed_char) == (-1))
		return (-1);
	len = hexalen(hex);
